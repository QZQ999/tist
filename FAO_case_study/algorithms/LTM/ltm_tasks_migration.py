...
class LTMTasksMigration:
    ...
    def task_migration(self) -> List[MigrationRecord]:
        """Execute task migration for LTM algorithm."""
        for robot_id in self.id_to_robots.keys():
            robot = self.id_to_robots[robot_id]
            if robot.fault_a == 1:
                tfs = list(robot.tasks_list)
                for task in tfs:
                    robot_migrated = self._greedy_find_migrated_robot(robot)
                    self._execute_migration(robot, robot_migrated, task)
        return self.records

    def _greedy_find_migrated_robot(self, f_robot: Robot) -> Robot:
        """Greedily find robot to migrate tasks to based on capacity/load ratio."""
        ...
        for neighbor_id in neighbors:
            ...
            if target_robot.load == 0:
                cratio = float('inf')
            else:
                cratio = target_robot.capacity / target_robot.load

            if cratio > max_cratio and target_robot.fault_a != 1:
                max_cratio = cratio
