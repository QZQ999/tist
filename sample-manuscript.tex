	...
	\begin{enumerate}
		...
		\item Comprehensive Empirical Validation Across Synthetic and Real-world Multiplex Systems. Extensive experiments show that the proposed method consistently outperforms existing multiagent task migration baselines in utility, completion ratio, and execution cost, while achieving solution quality that is comparable to optimal solvers at significantly lower computational cost. In addition, a real-world case study on the FAO international food trade network, which exhibits highly heterogeneous topologies, strong multilayer coupling, and uneven capacity distributions, confirms the practical robustness and scalability of the proposed framework in large-scale multiplex environments.
	\end{enumerate}
...
The FAO Multiplex Trade Network~\cite{nature} represents international trade relationships for food products among 214 countries, based on data from 2010. This dataset has been widely used in network science research and provides an authentic testbed for task migration algorithms in the context of global supply chains.
...
In addition to heuristic baselines, we evaluate two representative methods from multi-agent reinforcement learning (MARL), namely Multi-Agent Proximal Policy Optimization (MAPPO)~\cite{MAPPO} and Heterogeneous-Agent Proximal Policy Optimization (HAPPO)~\cite{HAPPO}. Both adopt a centralized-training and decentralized-execution paradigm that allows coordinated migration behavior to emerge through large-scale interaction. Although this paradigm provides strong modeling flexibility, the MARL baselines require prolonged training due to the high dimensionality of the continuous state space and the sparsity of effective reward signals. Their inference time during evaluation is minimal, yet their overall performance remains fundamentally constrained by the generalization capacity of policies learned from fixed training distributions.
...
Table~\ref{tab:fao_results} summarizes the normalized performance of all evaluated methods. Overall, HCTM-MPF consistently outperforms every practical baseline, achieving a survival rate extremely close to the theoretical optimum and exceeding the best heuristic competitor by a clear margin. This confirms that its multi-level potential–guided coordination effectively captures disruption propagation and mitigates cascading failures in heterogeneous trade networks.

Beyond heuristic approaches, we further benchmark two representative multi-agent reinforcement learning methods, MAPPO and HAPPO. Both achieve strong resilience, with MAPPO ranking second overall. Nevertheless, both learning-based approaches remain slightly inferior to HCTM-MPF in survival, indicating that although MARL policies internalize key structural cues of the FAO network, they struggle to generalize fully under asymmetric loads and varied disruption intensities.

The computational analysis highlights an even sharper contrast. Both MAPPO and HAPPO incur substantially higher online decision latency, operating more than twice as slow as HCTM-MPF. More importantly, they require extensive offline training which demands many hours on modern GPUs. Such heavy retraining costs severely limit their suitability for real-world environments where supply-chain conditions shift frequently. In contrast, HCTM-MPF requires no offline training and maintains stable performance across all runs, making it far more deployable in settings with dynamic operational patterns.

Cost-oriented performance further reinforces these observations. HCTM-MPF achieves a noticeably lower system cost, reducing overhead by more than ten percent relative to MAPPO and by an even larger margin compared to heuristic baselines. These improvements stem from its hierarchical clustering and potential-field design, which explicitly aligns local routing choices with system-level efficiency rather than relying on implicit learning from exploratory trajectories.

The load-balance results reveal a similar trend. HCTM-MPF achieves the most even workload distribution, improving balance by roughly four to five percentage points relative to MARL methods and by a substantially larger margin over heuristics. MARL approaches tend to over-utilize high-capacity hubs, reflecting partial overfitting to local reward signals during training. By contrast, HCTM-MPF’s leader–follower hierarchy integrates both capacity and centrality, preventing excessive concentration and maintaining robust network-wide utilization.

Across all metrics, HCTM-MPF achieves the most favorable trade-off. While MARL-based methods exhibit competitive accuracy under extensive training, their limited generalization and significantly higher runtime reduce their operational viability. HCTM-MPF therefore offers a more balanced, interpretable, and deployment-ready solution for task migration in complex, disruption-prone supply-chain environments.
...
MAPPO (1M episodes) & 5120$\pm$55 & 97.2$\pm$0.5 & 42.6$\pm$1.4 & 0.85$\pm$0.01 \\
HAPPO (1M episodes) & 6780$\pm$70 & 96.8$\pm$0.6 & 43.9$\pm$1.5 & 0.84$\pm$0.01 \\
...
\multicolumn{5}{l}{\small MAPPO/HAPPO: Multi-agent RL baselines trained for 1M episodes; runtime excludes offline training cost.} \\
...
To evaluate algorithmic robustness under varying computational loads, we conduct systematic scalability analysis across task counts ranging from 10 to 50. Each configuration is evaluated through 5 independent runs with different random seeds, reporting mean $\pm$ standard deviation to quantify statistical reliability. MARL baselines (MAPPO and HAPPO) are excluded from scalability analysis for two reasons.  First, their policies require fixed-size observation spaces and exhibit limited generalization beyond the state distribution encountered during training.  Extending the task count from 10 to 50 would require retraining from scratch, incurring over one million additional episodes per configuration.  Second, MARL training costs dominate total computation, making per-run scalability comparisons misleading.  Therefore, scalability analysis focuses on online execution efficiency, where heuristic methods and HCTM-MPF operate without retraining. Figure~\ref{fig:scalability} presents comprehensive scalability profiles across three critical dimensions.
